...
import com.example.newsproject.Adapter.TechnologyAdapter
...
import kotlinx.android.synthetic.main.maintechnology.*
...
class TechnologyActivity : AppCompatActivity()  , NavigationView.OnNavigationItemSelectedListener{
    ...
    val techadapter = TechnologyAdapter(list)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adView.adUnitId = "ca-app-pub-1351937667691042/6863435133"
        ...
        if (networkInfo != null && networkInfo.isConnected){
            GlobalScope.launch {
                val response = withContext(Dispatchers.IO) { Client.api.getTopHeadlinesTechnology() }
                if (response.isSuccessful) {
                    response.body()?.let { res ->
                        res.articles?.let { list.addAll(it)
                            Log.i("abc",it.toString())}
                        runOnUiThread { techadapter.notifyDataSetChanged() }
                    }
                }
            }
        }
