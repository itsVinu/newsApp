...
import kotlinx.android.synthetic.main.mainpolitics.*
...
class PoliticsActivity : AppCompatActivity() , NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recyclerView7.apply {
            ...
            adapter = politicadapter
        }

        politicadapter.onItemClick = {
        ...
        GlobalScope.launch {
            val response = withContext(Dispatchers.IO) { Client.api.getTopHeadlinesPolitics() }
            if (response.isSuccessful) {
                response.body()?.let { res ->
                    res.articles?.let { list.addAll(it)
                        Log.i("abc",it.toString())}
                    runOnUiThread { politicadapter.notifyDataSetChanged() }
                }
            }
        }
